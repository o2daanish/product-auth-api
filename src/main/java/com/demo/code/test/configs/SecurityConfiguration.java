...
public class SecurityConfiguration {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(auth -> auth.requestMatchers("/v1/auth/**").permitAll() // Open authentication endpoints
                        ...
                        .requestMatchers("/api/v1/products/**").hasAnyRole("USER", "ADMIN").anyRequest().authenticated())
                //.requestMatchers("/api/v1/products/admin/**").hasRole("ADMIN").anyRequest().authenticated())
